    ...
    const {
        ArduinoDataHumidity
    } = require('./serialHumidity')
    const {
        ArduinoDataSwitch
    } = require('./serialSwitch')
    const {
        ArduinoDataLuminosity
    } = require('./serialLuminosidity')
    ...
    router.get('/luminosity', (request, response, next) => {
        ...
        router.post('/sendData', (request, response) => {
            ...
            var sql = "INSERT INTO medidas (type, value) VALUES ('Proximidade',?)";

            db.query(sql, Switch, function (err, result) {
                if (err) throw err;
